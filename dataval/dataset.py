...
class WeatherDataset(object):
    ...
    def load_train(self, sample_frac=0.1):
        ...
        df = df.sort_values(by=["fact_time"], ascending=True).reset_index(drop=True)
        df["year_week"] = (
            df["fact_time"]
            .dt.isocalendar()
            .year.astype(str)
            .str.cat(
                df["fact_time"].dt.isocalendar().week.map("{:02d}".format).astype(str),
                sep="_",
            )
        )
    ...
    def corrupt_nonnegative(df, sensor_group, corruption_rate=0.1):
        summary = df.describe()
        sensor_cols = [
            col for col in df.columns if sensor_group in col and summary[col].min() >= 0
        ]
        copy = df.copy()
